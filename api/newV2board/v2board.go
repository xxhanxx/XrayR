...
func (c *APIClient) GetUserList() (UserList *[]api.UserInfo, err error) {
	...
	for i := 0; i < len(users); i++ {
		...
		u.HijackContent = users[i].HijackContent
		// Shadowsocks_0.0.0.0_10024|39edd651-ca07-4b37-8553-b1846f5d1b78@v2board.user|1|0|0
...
func (c *APIClient) ReportUserTraffic(userTraffic *[]api.UserTraffic) error {
	...
	for _, traffic := range *userTraffic {
		if traffic.Upload + traffic.Download > 100 * 1024 {
...
func (c *APIClient) ReportNodeOnlineUsers(onlineUserList *[]api.OnlineUser) error {
	...
	data := make([]api.OnlineUser, len(*onlineUserList))
	for i, user := range *onlineUserList {
		data[i] = api.OnlineUser{UID: user.UID, IP: user.IP}
	...
	path := "/api/v1/server/UniProxy/ip"
	res, err := c.client.R().
		SetBody(data).
		ForceContentType("application/json").
		Post(path)
